// pages/index/more/flowmeter/flowmeter.js
const app = getApp();
Page({
  ...
  bindKeyInput: function (e) {
    switch (e.currentTarget.id) {
      case 'lsValue':
        this.setData({
          lsValue: e.detail.value
        })
        break;
      case 'lxValue':
        this.setData({
          lxValue: e.detail.value
        })
        break;
      case 'dlValue':
        this.setData({
  ...
  validateData: function () {
    let cue = "";
    if (app.utils.isEmpty(this.data.lsValue)) {
      cue += "流量上限值不能为空！";
    } 
    if (app.utils.isEmpty(this.data.lxValue)) {
      cue += "流量下限值不能为空！";
    }
  ...
  compute: function (event) {
    ...
    if (app.utils.isEmpty(error)) {
      let Inputs = [];
      Inputs.push(this.data.lsValue, this.data.lxValue, this.data.dlValue);
