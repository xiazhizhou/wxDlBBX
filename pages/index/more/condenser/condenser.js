// pages/index/more/condenser/condenser.js
const app = getApp();
Page({
  ...
  validateData: function () {
    let cue = "";
    if (app.utils.isEmpty(this.data.qjValue)) {
      cue += "汽轮机背压不能为空！";
    } else {
      if (!(this.data.qjValue > 0)) {
        cue += "汽轮机背压不能小于0！";
      }
    }
    if (app.utils.isEmpty(this.data.dqValue)) {
      cue += "当地大气压不能为空！";
    } else {
      if (!(this.data.dqValue > 0)) {
        cue += "当地大气压不能小于0！";
      }
    }
    return cue;
  ...
  compute: function (event) {
    ...
    if (app.utils.isEmpty(error)) {
      let Inputs = [];
      Inputs.push(this.data.qjValue, this.data.dqValue);
