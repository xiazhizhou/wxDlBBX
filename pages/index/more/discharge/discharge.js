// pages/index/more/discharge/discharge.js
const app = getApp();
Page({
  ...
  validateData: function () {
    ...
    if (app.utils.isEmpty(this.data.pwValue)) {
      cue += "排污率不能为空！";
    }else {
      if (!(this.data.pwValue > 0)) {
        cue += "排污率不能小于0！";
      }
    }
    return cue;
  ...
  compute: function (event) {
    ...
    if (app.utils.isEmpty(error)) {
      let Inputs = [];
      Inputs.push(this.data.glValue, this.data.pwValue);
  ...
  advice(e){
    wx.navigateTo({
      url: '/pages/index/more/discharge/advice/advice'
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
