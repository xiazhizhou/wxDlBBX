// pages/index/more/datumconver/atmosphere2/atmosphere2.js
const app = getApp();
Page({
  ...
  validateData: function () {
    ...
    if (app.utils.isEmpty(this.data.sdjValue)) {
      ...
      if (!(this.data.sdjValue > 0 && this.data.sdjValue < 100)) {
        cue = "收到基水分不能超过取值范围！";
      }
    }
    if (app.utils.isEmpty(this.data.gzjValue)) {
      cue += "干燥基水分不能为空！";
    } else {
      if (!(this.data.gzjValue > 0)) {
        cue = "干燥基水分不能小于等于0！";
      }
    }
    return cue;
  ...
  compute: function (event) {
    ...
    if (app.utils.isEmpty(error)) {
      let Inputs = [];
      Inputs.push(this.data.sdjValue, this.data.gzjValue);
