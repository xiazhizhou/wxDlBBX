// pages/index/more/rateout/rateout.js
const app = getApp();
Page({
  ...
  validateData: function () {
    let cue = "";
    if (app.utils.isEmpty(this.data.rkValue)) {
      cue += "入口过空系数不能为空！";
    }else{
      if (!(this.data.rkValue > 0)) {
        cue = "入口过空系数不能超过取值范围！";
      }
    }
  ...
  compute: function (event) {
    ...
    if (app.utils.isEmpty(error)) {
      let Inputs = [];
      Inputs.push(this.data.rkValue, this.data.ckValue);
